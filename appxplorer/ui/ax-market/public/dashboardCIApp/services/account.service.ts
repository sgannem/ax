...
export class HttpAccountService {
  ...
  updateAccountDetails(updateAccountDetailsRequestDto: UpdateAccountDetailsRequestDto) {
    ...
    return this._http.put('http://localhost:8080/ax-ciservices/rest/ci/account/' + sessionStorage.getItem("ciexternaluserid"),
  ...
  getCountries() {
    let headers = new Headers({ 'Authorization': 'bearer ' + sessionStorage.getItem("ciUser") });
    let options = new RequestOptions({ headers: headers });
